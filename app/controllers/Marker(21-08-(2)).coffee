...
class Marker extends Spine.Controller
  ...
  processLine: (line) =>
    @lines.push line
    @visibility.push line.visibility
    @defPoints.push line.def if line.def?
  ...
  buildPoly : (lines) =>
    ...
    for i in [0..lines.length-1]
      poly.vertices.push @buildAnchor(100, i*20+200, i+'a') 
    ...
    if @noLines is 0
     ...
     @shapes.push circle
      ...
      for i in [1..@noLines]
        ...
        @shapes.push line 
        ###
        if not @noLines[i] instanceof Array
          line =  @buildLine(i)
          @shapes.push line 
        else 
          poly = @buildPoly(@noLines[i])
          @shapes.push poly
        ###
